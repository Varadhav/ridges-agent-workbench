...
# Enhanced polyglot support - language detection and configuration
SUPPORTED_LANGUAGES = {
    'python': {
        'extensions': ['.py'],
        ...
        'syntax_check': lambda content: compile(content, '<string>', 'exec')
    },
    'javascript': {
        'extensions': ['.js', '.jsx', '.mjs'],
        'test_patterns': ['*.test.js', '*.spec.js', 'test/*.js', '__tests__/*.js'],
        'test_function_prefix': ['test', 'it', 'describe'],
        'comment_prefix': '//',
        'import_patterns': [r'import\s+.*\s+from', r'require\s*\(', r'import\s*\('],
    ...
    'typescript': {
        ...
        'test_function_prefix': ['test', 'it', 'describe'],
        'comment_prefix': '//',
        'import_patterns': [r'import\s+.*\s+from', r'import\s*\('],
        'function_pattern': r'function\s+(\w+)\s*\(|(\w+)\s*=\s*\(',
        'class_pattern': r'class\s+(\w+)\s*{',
    ...
    'cpp': {
        ...
        'test_patterns': ['*test*.cpp', '*Test*.cpp', 'test/*.cpp', 'tests/*.cpp'],
        'test_function_prefix': 'TEST',
        'comment_prefix': '//',
        'import_patterns': [r'#include\s*[<"].*[>"]'],
        'function_pattern': r'[\w:]+\s+(\w+)\s*\([^)]*\)\s*{',
        'class_pattern': r'class\s+(\w+)',
        'compile_command': 'g++ -fsyntax-only',
        'syntax_check': None
    },
    'go': {
        'extensions': ['.go'],
        ...
        'test_function_prefix': 'Test',
        'comment_prefix': '//',
        'import_patterns': [r'import\s+\(', r'import\s+".*"'],
    ...
    'rust': {
        'extensions': ['.rs'],
        'test_patterns': ['*test*.rs', 'tests/*.rs'],
        'test_function_prefix': '#[test]',
        'comment_prefix': '//',
        'import_patterns': [r'use\s+[\w:]+;', r'extern\s+crate'],
        'function_pattern': r'fn\s+(\w+)\s*\(',
        'class_pattern': r'struct\s+(\w+)|enum\s+(\w+)',
        'compile_command': 'rustc --parse-only',
        'syntax_check': None
    }
}

def get_language_config(file_path: str = None, repo_dir: str = ".") -> dict:
    """Get language configuration for a file or detect from repository."""
    if file_path:
    ...
    # Fallback to detecting primary language
    primary_lang = detect_primary_language(repo_dir)
    return SUPPORTED_LANGUAGES.get(primary_lang, SUPPORTED_LANGUAGES['python'])

def detect_primary_language(repo_dir: str = ".") -> str:
...
class ParallelToolExecutor:
    ...
    def execute_parallel_analysis(self, file_path: str, test_func_names: List[str]) -> Dict[str, Any]:
        ...
        tasks = {
            'dependencies': lambda: self.tool_manager.analyze_dependencies(file_path) if hasattr(self.tool_manager, 'analyze_dependencies') else "Not available",
            'code_smells': lambda: self.tool_manager.detect_code_smells(file_path) if hasattr(self.tool_manager, 'detect_code_smells') else "Not available",
            'git_history': lambda: self.tool_manager.analyze_git_history(file_path) if hasattr(self.tool_manager, 'analyze_git_history') else "Not available",
            'code_quality': lambda: self.tool_manager.get_code_quality_metrics(file_path) if hasattr(self.tool_manager, 'get_code_quality_metrics') else "Not available"
        }
...
class ToolManager:
    """Base tool manager class with core functionality"""
    TOOL_LIST = {}
    ...
    def check_syntax_error(self, content: str, file_path: str = "<unknown>") -> tuple[bool, Optional['ToolManager.Error']]:
        """Check syntax error for multiple programming languages."""
        try:
            lang_config = get_language_config(file_path)
            
            # Python syntax checking
            if any(file_path.endswith(ext) for ext in SUPPORTED_LANGUAGES['python']['extensions']):
                ast.parse(content, filename=file_path)
                return False, None
            
            # JavaScript/TypeScript - basic syntax check
            elif any(file_path.endswith(ext) for ext in SUPPORTED_LANGUAGES['javascript']['extensions'] + SUPPORTED_LANGUAGES['typescript']['extensions']):
            ...
            # Java - basic syntax check
            elif any(file_path.endswith(ext) for ext in SUPPORTED_LANGUAGES['java']['extensions']):
            ...
            # C++ - basic syntax check
            elif any(file_path.endswith(ext) for ext in SUPPORTED_LANGUAGES['cpp']['extensions']):
                if content.count('{') != content.count('}'):
                    raise SyntaxError("Mismatched braces")
                if content.count('(') != content.count(')'):
                    raise SyntaxError("Mismatched parentheses")
            ...
            elif any(file_path.endswith(ext) for ext in SUPPORTED_LANGUAGES['go']['extensions']):
                if content.count('{') != content.count('}'):
                    raise SyntaxError("Mismatched braces")
                if content.count('(') != content.count(')'):
                    raise SyntaxError("Mismatched parentheses")
            ...
            # Rust - basic syntax check
            elif any(file_path.endswith(ext) for ext in SUPPORTED_LANGUAGES['rust']['extensions']):
                if content.count('{') != content.count('}'):
                    raise SyntaxError("Mismatched braces")
                if content.count('(') != content.count(')'):
                    raise SyntaxError("Mismatched parentheses")
            ...
            # Default: assume valid for unknown file types
            return False, None
            
        except SyntaxError as e:
            logger.error(f"Syntax error in {file_path}: {e}")
            return True, ToolManager.Error(ToolManager.Error.ErrorType.SYNTAX_ERROR,f"Syntax error. {str(e)}")

    def is_test_file(self, file_path: str) -> bool:
        """Check if a file is a test file for multiple programming languages."""
        ...
        # Fallback to generic patterns for any language
        generic_test_patterns = [
            r".*test.*\.(py|js|ts|java|cpp|cc|cxx|go|rs)$",
            r".*Test.*\.(py|js|ts|java|cpp|cc|cxx|go|rs)$",
            r".*spec.*\.(py|js|ts|java|cpp|cc|cxx|go|rs)$",
        ]
        
        # Check generic filename patterns
        ...
        test_directories = ["tests", "testing", "test", "__tests__", "spec"]
        if any(test_dir in parts for test_dir in test_directories):
            return True
...
class EnhancedToolManager(ToolManager):
        ...
        try:
            ...
            for node in ast.walk(ast.parse(content)):
                if isinstance(node, (ast.Import, ast.ImportFrom)):
                    dependencies['imports'].append(node.module if isinstance(node, ast.ImportFrom) else node.names[0].name)
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.DEPENDENCY_ANALYSIS_ERROR, 
    ...
    def detect_code_smells(self, file_path: str) -> str:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    if node.body and len(node.body) > 20:  # Arbitrary threshold
                        smells.append(f"Long function: {node.name} (lines {node.lineno}-{getattr(node, 'end_lineno', node.lineno)})")
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.CODE_SMELL_DETECTION_ERROR, 
    ...
    def get_code_quality_metrics(self, file_path: str) -> str:
        ...
        '''
        try:
            # Basic code quality metrics without external dependencies
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.CODE_QUALITY_ERROR, 
    ...
    def analyze_git_history(self, file_path: str, commit_range: str = "HEAD~5..HEAD") -> str:
        ...
        try:
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.GIT_HISTORY_ERROR, 
...
class CreateTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def __init__(self, available_tools: Optional[list[str]] = None, test_cases: list[dict] = [], test_file: str = "custom_tests", language: str = PROBLEM_LANGUAGE_PYTHON):
        ...
        self.tool_failure={
            k:{j:0 for j in ToolManager.Error.ErrorType.__members__} for k in self.TOOL_LIST.keys()
        }
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def _get_file_content(
        ...
        - If line range is provided, adjusts to function boundaries for Python files.
        ...
        # Adjust start/end lines if they fall within a function (Python files only)
        if file_path.endswith('.py'):
            try:
                ...
                # If function range detection fails, continue without adjustment
                pass
    ...
    def get_file_content(self,file_path: str, search_start_line: int = None, search_end_line: int = None, search_term: str = None)->str:
        ...
        Supports multiple programming languages (Python, JavaScript, TypeScript, Java, C++, Go, Rust).
        ...
        Arguments:
            file_path: filesystem path to target file. Supports multiple programming languages.
        ...
        try:
            ...
            lang_config = get_language_config(file_path)
            if not any(file_path.endswith(ext) for lang_config in SUPPORTED_LANGUAGES.values() for ext in lang_config['extensions']):
                logger.info(f"File {file_path} may not be a supported programming language file, but attempting to read it anyway")
    ...
    def save_file(self,file_path: str, content: str)->str:
        ...
        if "test" in file_path.lower() or "reproduce" in file_path.lower():
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL,f"Error: You cannot use this tool to create test or files to reproduce the error.")
    ...
    def get_approval_for_solution(self,solutions:list[str],selected_solution:int,reason_for_selection:str)->str:
        ...
        if type(solutions) is not list or len(solutions)<2:
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL,f"Error: solutions must be a list with length at least 2.")
    ...
    def _save(self,file_path: str, content: str)->str:
        ...
        if not is_syntax_error:
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.SYNTAX_ERROR,error.message)
    ...
    def search_in_all_files_content(self,search_term: str)->str:
        '''
        Search for a text pattern across all supported language files in the project (Python, JavaScript, TypeScript, Java, C++, Go, Rust).
        ...
        Arguments:
            search_term: text pattern to locate (e.g., "def test_function", "*SomeClass*", "function myFunc")
        Output:
        ...
        # Walk through all directories and find supported files
        for root, _, files in os.walk("."):
                    ...
                    try:
                        ...
                        if file_path.endswith('.py'):
                            try:
                                ...
                                # Fallback to simple line search for Python files with syntax errors
                                lines = content.split('\n')
                                for line_num, line in enumerate(lines, 1):
                                    if re.search(search_term, line, re.IGNORECASE):
                                        output.append(f"{file_path}:{line_num} | {line.rstrip()}")
                        else:
                            # For non-Python files, use simple line search
                            lines = content.split('\n')
                            for line_num, line in enumerate(lines, 1):
                                if re.search(search_term, line, re.IGNORECASE):
                                    output.append(f"{file_path}:{line_num} | {line.rstrip()}")
        ...
        if not output:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND,f"'{search_term}' not found in the codebase.")
        return output

    @ToolManager.tool
    def search_in_all_files_content_v2(self, grep_search_command: str, test_files_only: bool = False) -> str:
        '''
        Performs grep -rnE search across all files in the codebase (supports multiple programming languages)
        Tips:
        ...
        Arguments:
            ...
            if test_files_only is True, then add appropriate test file patterns for detected languages.
        ...
        # Detect primary language and get all supported extensions
        ...
        if test_files_only:
            # Add test file patterns for all supported languages
            test_patterns = []
            for lang, config in SUPPORTED_LANGUAGES.items():
                for pattern in config['test_patterns']:
                    test_patterns.append(f"--include='{pattern}'")
            
            # Add generic test patterns
            generic_test_patterns = [
                "--include='*test*'",
                "--include='*Test*'", 
                "--include='*spec*'",
            ...
            test_patterns.extend(generic_test_patterns)
            
            # Remove existing Python-specific includes
            grep_search_command = grep_search_command.replace("--include='*test*.py'", "")
            grep_search_command = grep_search_command.replace("--include='*.py'", "")
            
            # Add all test patterns
            grep_search_command += " " + " ".join(test_patterns)
        else:
            # Add include patterns for all supported file extensions
            if not any(f"--include=" in grep_search_command for ext in all_extensions):
                include_patterns = []
                for ext in all_extensions:
                    include_patterns.append(f"--include='*{ext}'")
                grep_search_command += " " + " ".join(include_patterns)
        ...
        if not output:
            ...
            detected_lang = f" (detected primary language: {primary_lang})"
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND, 
                                f"'{grep_search_command}' not found in {file_type}{detected_lang}.")
        return output
    ...
    def get_function_ranges(self,file_path: str)->list[tuple[int, int, str]]:
        ...
        try:
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.FILE_NOT_FOUND,f"Error reading '{file_path}': {e}")
        try:
            tree = ast.parse("\n".join(source_lines), filename=file_path)
        except SyntaxError as e:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SYNTAX_ERROR,f"Error parsing '{file_path}': {e}, {traceback.format_exc()}")
    ...
    def _extract_function_matches(self,file_path: str, search_term: str, *, max_output_lines: int = 1000) -> str:
        ...
        if not match_lines:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND,f"'{search_term}' not found in file '{file_path}'")
    ...
    def _extract_text_matches(self, file_path: str, search_term: str, *, max_output_lines: int = 1000) -> str:
        '''
        Return lines containing search_term for non-Python files.
        Used for JavaScript, TypeScript, Java, C++, Go, Rust, etc.
        '''
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                source_lines = f.read().splitlines()
        ...
        for idx, line in enumerate(source_lines):
            if search_term.lower() in line.lower():
                match_lines.append(f"{idx + 1}:{line}")

        if not match_lines:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND,
                f"'{search_term}' not found in file '{file_path}'")

        return Utils.limit_strings("\n".join(match_lines), n=max_output_lines)

    @ToolManager.tool
    def search_in_specified_file_v2(self,file_path: str, search_term: str)->str:
        '''
        Locates text patterns within a specific file (supports multiple programming languages)
        Arguments:
            file_path: target file for pattern matching. Supports Python, JavaScript, TypeScript, Java, C++, Go, Rust files.
            search_term: text pattern to find (e.g., "def test_function", "class SomeClass", "function myFunc")
        Output:
        ...
        if not any(file_path.endswith(ext) for ext in supported_extensions):
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_FILE_PATH,
                f"Error: file '{file_path}' is not a supported programming language file. Supported extensions: {', '.join(supported_extensions)}")
        
        # For Python files, use the existing function extraction method
        if file_path.endswith('.py'):
            return self._extract_function_matches(file_path, search_term)
        
        # For other languages, use simple text search with line numbers
        return self._extract_text_matches(file_path, search_term)
    ...
    def search_recurive_in_all_files_in_directory(self, directory_path: str, search_term: str)->str:
        ...
        if not os.path.exists(directory_path) or not os.path.isdir(directory_path):
            raise ToolManager.Error(ToolManager.Error.ErrorType.FILE_NOT_FOUND,f"Error: directory '{directory_path}' does not exist.")
        ...
        if not output:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND,f"'{search_term}' not found in file '{directory_path}'")
        return output
    ...
    def run_code(self,content:str,file_path:str)->str:
        '''
        Runs code in multiple programming languages (Python, JavaScript, TypeScript, Java, C++, Go, Rust).
        ...
        Output:
            ...
            Returns error message if there are any third party dependencies or compilation errors.
        '''
        try:
            self._save(file_path, content)
        except Exception as e:
            raise ToolManager.Error(ToolManager.Error.ErrorType.SYNTAX_ERROR,f"Error saving code: {e}\n")
    
        # Determine language and execution method
        lang_config = get_language_config(file_path)
        
        if file_path.endswith('.py'):
            # Python execution (existing logic)
            with open(file_path, "r") as f:
            ...
            result = subprocess.run(["node", file_path], capture_output=True, text=True, check=False, timeout=60)
        
        elif file_path.endswith('.ts'):
            # TypeScript execution (requires ts-node or compilation)
            # Try ts-node first, fallback to tsc + node
            ts_node_result = subprocess.run(["which", "ts-node"], capture_output=True, text=True)
            if ts_node_result.returncode == 0:
                result = subprocess.run(["ts-node", file_path], capture_output=True, text=True, check=False, timeout=60)
            else:
                # Compile and run
                js_file = file_path.replace('.ts', '.js')
                compile_result = subprocess.run(["tsc", file_path, "--outFile", js_file], capture_output=True, text=True, timeout=30)
                if compile_result.returncode == 0:
                    result = subprocess.run(["node", js_file], capture_output=True, text=True, check=False, timeout=60)
            ...
            compile_result = subprocess.run(["javac", file_path], capture_output=True, text=True, timeout=30)
            if compile_result.returncode == 0:
                result = subprocess.run(["java", class_name], capture_output=True, text=True, check=False, timeout=60)
                # Clean up class file
            ...
            compile_result = subprocess.run(["g++", "-o", exe_name, file_path], capture_output=True, text=True, timeout=30)
            if compile_result.returncode == 0:
                result = subprocess.run([f"./{exe_name}"], capture_output=True, text=True, check=False, timeout=60)
                # Clean up executable
            ...
            # Go execution
            result = subprocess.run(["go", "run", file_path], capture_output=True, text=True, check=False, timeout=60)
        
        elif file_path.endswith('.rs'):
            # Rust execution (compile + run)
            exe_name = os.path.splitext(file_path)[0]
            compile_result = subprocess.run(["rustc", "-o", exe_name, file_path], capture_output=True, text=True, timeout=30)
            if compile_result.returncode == 0:
                result = subprocess.run([f"./{exe_name}"], capture_output=True, text=True, check=False, timeout=60)
                # Clean up executable
                if os.path.exists(exe_name):
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.RUNTIME_ERROR,
                f"Unsupported file type: {file_path}. Supported: .py, .js, .ts, .java, .cpp, .go, .rs")
        ...
        if result.returncode != 0:
            ...
            elif any(file_path.endswith(ext) for ext in ['.js', '.ts']):
                if "Cannot find module" in error_output or "MODULE_NOT_FOUND" in error_output:
                    error_type = ToolManager.Error.ErrorType.THIRD_PARTY_DEPENDENCIES
            elif file_path.endswith('.java'):
                if "package does not exist" in error_output or "cannot find symbol" in error_output:
                    error_type = ToolManager.Error.ErrorType.THIRD_PARTY_DEPENDENCIES
            
            raise ToolManager.Error(error_type, f"Error running {file_path}: {error_output}\n")
        
        if len(result.stdout) == 0:
            observation = f"Congratulations! Code executed successfully with no output."
        else:
    ...
    def apply_code_edit(self,file_path:str, search:str, replace:str)->str:
        ...
        if not self.is_solution_approved:
            raise ToolManager.Error(ToolManager.Error.ErrorType.INVALID_TOOL_CALL,f"Error: You cannot use this tool before you have approval from user on your proposed solution. Please call get_approval_for_solution tool first with list of proposed solutions.")
        if not os.path.exists(file_path):
            logger.error(f"file '{file_path}' does not exist.")
            raise ToolManager.Error(ToolManager.Error.ErrorType.FILE_NOT_FOUND,f"Error: file '{file_path}' does not exist.")
        ...
        match original.count(search):
            case 0:
                ...
                raise ToolManager.Error(ToolManager.Error.ErrorType.SEARCH_TERM_NOT_FOUND,f"Error: search string not found in file {file_path}. You need to share the exact code you want to replace.")
            case 1:
                ...
                try:
                    ...
                    raise ToolManager.Error(ToolManager.Error.ErrorType.SYNTAX_ERROR,f"Error: syntax error in file {file_path}. {e.message}")
            case num_hits:
                logger.error(f"search string found {num_hits} times in file '{file_path}'.\nPlease reformulate your search and replace to apply only one change.")
                raise ToolManager.Error(ToolManager.Error.ErrorType.MULTIPLE_SEARCH_RESULTS_FOUND,f"Error: search string found {num_hits} times in file '{file_path}'.\nPlease reformulate your search and replace to apply only one change.")
    
    @ToolManager.tool
    def compile_repo(self) -> str:
        '''
        Compile/check syntax for all source files in multiple programming languages.
        Arguments:
            None
        Output:
        ...
        try:
            ...
            if not all_files:
                return f"No source files found for supported languages. Primary language detected: {primary_lang}"
            ...
            # Compile/check each language
            for lang, files in files_by_lang.items():
                lang_config = SUPPORTED_LANGUAGES[lang]
                compile_cmd = lang_config.get('compile_command')
                
                if not compile_cmd:
                    compile_results.append(f"{lang.upper()}: Skipped (no compile command configured)")
                    continue
                ...
                if lang == 'python':
                    ...
                    if proc.returncode != 0:
                        compile_results.append(f"PYTHON COMPILE ERRORS:\n{proc.stderr or proc.stdout}")
                    else:
                        compile_results.append(f"PYTHON: OK ({len(files)} files)")
                    ...
                    if node_check.returncode == 0:
                        ...
                        for file_path in files:
                            proc = subprocess.run(["node", "--check", file_path], capture_output=True, text=True, timeout=10)
                            if proc.returncode != 0:
                                errors.append(f"{file_path}: {proc.stderr}")
                        if errors:
                            compile_results.append(f"JAVASCRIPT SYNTAX ERRORS:\n" + "\n".join(errors))
                        else:
                            compile_results.append(f"JAVASCRIPT: OK ({len(files)} files)")
                    else:
                        compile_results.append("JAVASCRIPT: Skipped (node not available)")
            
            # Check if any errors occurred
            if any("ERROR" in result for result in compile_results):
                return "\n".join(compile_results)
            else:
                return f"COMPILE CHECK OK\nPrimary language: {primary_lang}\n" + "\n".join(compile_results)
    ...
    def enhanced_problem_analysis(self, problem_statement: str) -> str:
        '''
        Enhanced problem analysis with comprehensive insights
        Arguments:
            problem_statement: The problem to analyze comprehensively
        Output:
            Detailed analysis with recommendations and confidence scores
        '''
        try:
            ...
            analysis = {
                ...
                'language_hints': self._detect_language_hints(problem_statement),
                'urgency_level': self._assess_urgency(problem_statement)
            }
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.UNKNOWN, 
    ...
    def _detect_language_hints(self, problem: str) -> List[str]:
        """Detect programming language hints in the problem statement"""
        hints = []
        for lang, config in SUPPORTED_LANGUAGES.items():
            for ext in config['extensions']:
                if ext in problem:
    ...
    def _assess_urgency(self, problem: str) -> str:
        """Assess the urgency level of the problem"""
        urgent_keywords = ['critical', 'urgent', 'emergency', 'broken', 'crash', 'fail']
        problem_lower = problem.lower()
        
        if any(keyword in problem_lower for keyword in urgent_keywords):
            return 'high'
        elif any(keyword in problem_lower for keyword in ['bug', 'error', 'issue']):
            return 'medium'
        else:
            return 'low'
    ...
    def _generate_recommendations(self, analysis: dict) -> List[str]:
        ...
        problem_type = analysis.get('problem_type', 'general_issue')
        complexity = analysis.get('complexity_level', 'medium')
        
        if problem_type == 'testing_debugging':
            recommendations.extend([
                'Start by running existing tests to understand current failures',
                'Use search tools to locate relevant test files',
                'Analyze test output for specific error patterns'
            ])
        elif problem_type == 'dependency_issue':
            recommendations.extend([
                'Check import statements and module availability',
                'Verify PYTHONPATH and environment setup',
                'Look for circular dependencies'
            ])
        elif problem_type == 'syntax_error':
            recommendations.extend([
                'Use compile_repo tool to check syntax across all files',
                'Focus on recently modified files',
                'Check for language-specific syntax requirements'
            ])
        
        if complexity == 'complex':
            recommendations.append('Consider breaking the problem into smaller, manageable parts')
    ...
    def _calculate_confidence(self, analysis: dict) -> float:
        ...
        problem_type = analysis.get('problem_type', 'general_issue')
        if problem_type in ['testing_debugging', 'syntax_error']:
            base_confidence += 0.1
        
        return min(max(base_confidence, 0.0), 1.0)

    def _estimate_effort(self, analysis: dict) -> str:
        """Estimate effort required based on analysis"""
        complexity = analysis.get('complexity_level', 'medium')
        problem_type = analysis.get('problem_type', 'general_issue')
        
        if complexity == 'simple' and problem_type in ['syntax_error']:
            return 'low'
        elif complexity == 'complex' or problem_type == 'dependency_issue':
            return 'high'
        else:
            return 'medium'
    ...
    def finish(self,investigation_summary: str):
        ...
        if qa_response.get("is_patch_correct","no").lower()=="yes":
            ...
            raise ToolManager.Error(ToolManager.Error.ErrorType.BUG_REPORT_REQUIRED,qa_response.get("analysis",""))
